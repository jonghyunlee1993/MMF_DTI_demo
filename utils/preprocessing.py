    ...
    if use_gpt == "Yes" and len(api_key) > 0:
        ...
        model="gpt-3.5-turbo",
            messages=[
                ...
                {"role": "user", "content": f"Read the given text and shorten it. The maximum length of the summarization should not exceed 512 characters. The output format should follow the output template.\n\nOUTPUT TEMPLATE\n[OUTPUT] [Write the summarized text]\n\nINPUT TEXT: {text}"}
            ],
            temperature=0,
            max_tokens=512,
            top_p=1,
...
def tokenize_inputs(SMILES, FASTA, function_description, mol_tokenizer, prot_tokenizer, text_tokenizer, use_gpt, api_key):
    tokenized_smiles = mol_tokenizer(SMILES, max_length=512, truncation=True, return_tensors="pt")
    tokenized_fasta = prot_tokenizer(" ".join(FASTA), max_length=545 + 2, truncation=True, return_tensors="pt")
    ...
    if function_description != None:
        ...
        text = "Not applicable."
    tokenized_text = text_tokenizer(text, max_length=512, truncation=True, return_tensors="pt")
...
def unsqueeze_everything(tokenized_smiles, tokenized_fasta, prot_feat_teacher, tokenized_text, mol_property):
    ...
    return tokenized_smiles, tokenized_fasta, prot_feat_teacher, tokenized_text, mol_property