...
def load_models(prot_tokenizer):
    ...
    dti_model.load_state_dict(
        torch.load("weights/MMF_DTI.pt", map_location=torch.device("cpu")), strict=False
    )
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column():
                ...
                functional_description = gr.Textbox(
                    label="Target Function Description", lines=3
                )
                use_gpt = gr.Radio(
                    label="Use GPT for summarization", choices=["Yes", "No"], value="No"
                )
        ...
        btn.click(
            ...
            inputs=[SMILES, FASTA, functional_description, use_gpt, api_key],
            outputs=output,
        )
